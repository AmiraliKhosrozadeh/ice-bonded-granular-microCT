...
# -- Settings (SCAN 2 — failure / compression scan) ---------------------------
# Raw stack: 847 slices (0000..0846), X is the stacking axis in the TIFFs
FIRST_SLICE = 88     # first slice where particles appear (scan-2 indexing)
LAST_SLICE  = 809    # last slice where particles appear (722 slices total)

TIFF_DIR    = r'E:\RPTU-images\CT_images\Glass\Glass_100_1800_T5_02'
FILE_PREFIX = 'Glass-1001800-T5-normal_scan2_4_100XXL_uc_xy_'

# Wide X/Y crop — covers the specimen in BOTH scans so compression-induced
# drift doesn't chop off migrated beads. Must match scan 1's SPAM run AND
# Dragonfly's load bounds (minX=469, maxX=1045, minY=48, maxY=633).
# Python slice is half-open [a:b], Dragonfly is inclusive [a..b], so the
# Python lower bound equals Dragonfly's min and upper bound = Dragonfly max+1.
X_MIN, X_MAX = 469, 1046   # 577 voxels wide (matches Dragonfly)
Y_MIN, Y_MAX = 48,  634    # 586 voxels tall (matches Dragonfly)
...
# Cylinder parameters (world coordinates in um) — MEASURE FRESH IN DRAGONFLY
...
#   4) Paste them here.
CYL_CAP1_UM   = (18723.10, 8408.06,   229.11)   # scan 2 (updated 2026-04-20)
CYL_CAP2_UM   = (18723.10, 8408.06, 17694.89)   # scan 2 — vertical (no tilt)
CYL_RADIUS_UM = 6601.14                         # scan 2 — user-drawn cylinder

# Safety margin on the cylinder radius — disabled to honor the user-drawn
...
# scan-2 load (minX=469, minY=48, minZ=0) on 2026-04-20. Identical to scan 1
# because Dragonfly reports origin relative to each channel's own voxel 0.
...
OUT_DIR = r'E:\RPTU-images\CT_images\Glass\Glass_scan02_dragonfly\segmentation'
...
# Thresholds (scan 2 — tuned against user-verified voxel values 2026-04-20)
# Scan 2 peaks: 3448 (outside air), 6942 (air/ice mix), 8550 (ice), 24538 +
# 26239 (glass double peak from beam hardening), 34712 (glass tail, NOT Al).
# User-verified ground truth:
#   10220 -> ice   (so T_ICE_GLASS must be > 10220)
#   36737 -> glass (so T_GLASS_AL must be > 36737; Al extends up to ~61849)
T_AIR_ICE   = 6200    # user-tuned threshold (prev 8000 too high, 5195 too low)
T_ICE_GLASS = 16544   # mid(8550, 24538) — ice vs glass boundary
T_GLASS_AL  = 37000   # user-tuned for scan 2 (lowered from 45000)
...
# Write ct_scan02.tif and specimen_mask_scan02.tif into Glass_spam\data\ so the
...
try:
    ct_tif   = os.path.join(SPAM_DATA_DIR, 'ct_scan02.tif')
    mask_tif = os.path.join(SPAM_DATA_DIR, 'specimen_mask_scan02.tif')
