"""
Porosity analysis for CT segmented specimen.
Run AFTER segmentation.py (uses seg + specimen_mask in memory).
...
# Labels (must match segmentation.py)
LABEL_OUTSIDE_AIR = 0
LABEL_TRAPPED_AIR = 1
LABEL_ICE         = 2
LABEL_GLASS       = 3
...
# Cylinder parameters (keep in sync with scan02_segmentation.py).
# TODO: update these to the scan-2 cylinder caps measured in Dragonfly.
CYL_CAP1_UM = (19501.05, 8206.22,   233.13)
CYL_CAP2_UM = (18737.14, 8178.46, 21259.05)
CYL_RADIUS_UM = 5256.61
CT_ORIGIN_UM = (13980.41, 2885.24, -12.38)
...
if 'seg' not in dir():
    raise RuntimeError("Run segmentation.py first!")

# specimen_mask is the strict interior (ice + glass + trapped air region).
# Fall back to building one from the seg labels if segmentation.py didn't
# leave it in memory (e.g. running this script in isolation on saved TIFFs).
if 'specimen_mask' in dir():
    print("Using specimen_mask from segmentation.py (contour or cylinder mode).")
else:
...
print("=" * 60)
print("Porosity Analysis — interior (ice + glass + trapped air)")
print("=" * 60)
...
for z in range(nz):
    ...
    # 'is_specimen' = voxels that actually have a valid phase label
    # (excludes partial-volume boundary voxels that stayed LABEL_OUTSIDE_AIR
    # inside specimen_mask; otherwise the outermost radial bin would see
    # specimen_count > 0 but all phase counts == 0 -> fractions drop to 0).
    is_specimen = is_ice | is_air | is_glass
    is_pore     = is_ice | is_air

    # ---- radial coordinate -------------------------------------------------
    if RADIAL_METHOD == 'distance_transform':
        # True distance to the nearest specimen edge, per slice. Use the
        # LABELED specimen (is_specimen) as the shape so the radius origin
        # sits at the deepest labeled interior and r=1 is the outer boundary
        # of real material (not the fuzzy specimen_mask boundary).
        base = is_specimen if is_specimen.any() else inside
        d_edge = ndimage.distance_transform_edt(base)
        d_max = d_edge.max()
        if d_max <= 0:
            continue
    ...
    for b in range(N_RADIAL_BINS):
        mb = is_specimen & (bin_idx == b)   # labeled-specimen voxels only
    ...
    height['specimen'][h_bin]   += np.sum(is_specimen)   # labeled-only
...
MIN_BIN_VOXELS = 500   # suppress fractions from bins with too few voxels (noise floor)
def safe_div(a, b):
    return np.where(b >= MIN_BIN_VOXELS, a / np.maximum(b, 1), 0)
...
with open(stats_path, 'w') as f:
    f.write("Segmentation Statistics (aluminum excluded from analysis)\n")
    f.write("=" * 65 + "\n")
...
with open(radial_path, 'w') as f:
    f.write(f"Radial Porosity Profiles (method='{RADIAL_METHOD}', fraction of interior per radial bin)\n")
...
with open(height_path_data, 'w') as f:
    f.write("Height Porosity Profiles (fraction of interior per height bin)\n")
...
with open(desc_path, 'w') as f:
    f.write("POROSITY ANALYSIS — DEFINITIONS AND FILES\n")
    f.write("=" * 60 + "\n\n")
    f.write("Glass bead / ice / trapped air CT specimen.\n")
    ...
    f.write("Given by specimen_mask from segmentation.py.\n\n")
...
plt.title('Radial Profiles — All Fractions', fontsize=FS, fontweight='bold', pad=18)
plt.tight_layout()
