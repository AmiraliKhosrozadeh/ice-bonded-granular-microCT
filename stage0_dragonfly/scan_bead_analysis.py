...
MARKER_MIN_DIST  = 10        # min distance (voxels) between bead centers.
                             # Only used when MARKER_METHOD='peak'.

# Marker-finding method. 'cc' uses connected components on the eroded mask —
# if erosion broke the bridge between two beads, they're guaranteed separate
# markers. This is MORE robust than peak_local_max for touching particles,
# which can collapse two nearby peaks into one. Scan 2: 'cc' recommended.
MARKER_METHOD = 'cc'        # 'cc' | 'peak'
MARKER_CC_MIN_VOXELS = 20   # CC markers smaller than this are discarded (noise)
MORPH_CLOSING_R  = 0        # 0 = no closing (closing glues beads together!)
DIST_SMOOTH_SIGMA = 1.0     # smooth distance map before peak detection.
                            # Lowered from 1.5 to recover touching-bead splits.
...
USE_FERET_FOR_PSD  = False  # False -> use equivalent-sphere diameter (matches Camsizer x_vol)
# Options for the PSD x-axis diameter:
#   False (default) -> equivalent-sphere diameter (from voxel volume, Camsizer x_vol)
#   True            -> MEAN Feret diameter over 13 unit directions (paper Section 3.5.4)

# PSD QC filter — merged-bead clusters have Feret >> equivalent diameter
# (elongated shape). A true sphere has Feret/Equivalent ≈ 1.0–1.2. Any bead
# with ratio above PSD_MAX_FERET_RATIO is flagged as a merger and excluded
# from the PSD plot + D10/D50/D90 to keep stats meaningful. Still exported
# in bead_measurements.csv so nothing is silently lost.
PSD_MAX_FERET_RATIO = 1.30    # tightened from 1.5 to drop merged disc-shaped beads
PSD_DIAM_MAX_UM    = 2400    # hard upper bound (um); any bead above this is dropped from PSD
...
# SPAM export target for bead labels
SPAM_DATA_DIR    = r'E:\RPTU-images\CT_images\Glass\<SPECIMEN>\<SPAM>\data'
SPAM_LABELS_PATH = os.path.join(SPAM_DATA_DIR, 'bead_labels_scan01.tif')
...
ERODE_ITERATIONS = 24   # voxels to erode. Higher = more aggressive splitting.
                        # Scan 2: 24 (prev 20) to snap small-voxel bridges between
                        # touching compressed beads.
...
if HAS_SKIMAGE:
    if MARKER_METHOD == 'cc':
        # Connected components on the eroded mask. Each CC = one bead.
        # Robust for touching beads: if erosion broke the bridge, the two
        # beads are guaranteed separate markers (no peak-detection ambiguity).
        markers, n_markers_raw = ndimage.label(glass_mask_eroded)
        # Drop markers below size threshold (erosion artifacts / noise).
        if MARKER_CC_MIN_VOXELS > 0:
            counts = np.bincount(markers.ravel())
            keep = counts >= MARKER_CC_MIN_VOXELS
            keep[0] = True  # background stays 0
            remap = np.zeros(len(counts), dtype=np.int32)
            new_id = 0
            for old_id in range(len(counts)):
                if old_id == 0:
                    continue
                if keep[old_id]:
                    new_id += 1
            ...
            n_markers = n_markers_raw
        print(f"  CC markers (MARKER_METHOD='cc'): {n_markers} "
              f"(raw={n_markers_raw}, dropped {n_markers_raw-n_markers} < "
              f"{MARKER_CC_MIN_VOXELS} vox)")
        print(f"  Found {n_markers} bead markers in {time.time()-t0:.1f}s")
        coords = np.argwhere(markers > 0)  # for QC plot only; not used downstream
    else:
        # Legacy peak-based markers (can collapse touching-bead peaks).
...
import sys
sys.path.insert(0, r'E:\RPTU-images\CT_images\Glass\<SPECIMEN>\\scan{N}_dragonfly')
...
def min_feret_diameter_um(coords, voxel_um=VOXEL_SIZE_UM, dirs=_FERET_DIRS):
    """Minimum Feret diameter in um -- closest to Camsizer x_Fe_min (sieve size).
    Returns the SMALLEST caliper diameter across the sampled directions."""
    projs = coords @ dirs.T
    ranges_vox = projs.max(axis=0) - projs.min(axis=0) + 1.0
    return float(np.min(ranges_vox)) * voxel_um
...
for bead_id in range(1, n_beads + 1):
    ...
    # Mean and min Feret diameters (paper Section 3.5.4).
    # feret_min_um is the Camsizer-x_Fe_min analogue (sieve-like size).
    feret_mean_um = mean_feret_diameter_um(coords_i)
    feret_min_um  = min_feret_diameter_um(coords_i)
...
if USE_FERET_FOR_PSD:
    diams_full = np.array(feret_all)
    diam_label = "Mean Feret diameter"
    psd_title_suffix = " (Feret's diameter)"
else:
    diams_full = np.array(diams_all)
    diam_label = "Equivalent diameter"
    psd_title_suffix = ""
vols_full  = np.array([r['volume_mm3'] for r in results])

# QC filter: drop merged-bead clusters (Feret >> equivalent diameter) so a
# few outliers don't dominate the PSD bin layout and squash the real
# distribution into one bin.
eq_arr = np.array(diams_all)   # always equivalent diameter
feret_arr = np.array(feret_all)
ratio = feret_arr / np.maximum(eq_arr, 1.0)
psd_keep = (ratio <= PSD_MAX_FERET_RATIO) & (diams_full <= PSD_DIAM_MAX_UM) & (eq_arr <= PSD_DIAM_MAX_UM)
n_drop = int((~psd_keep).sum())
if n_drop > 0:
    print(f"  PSD filter: excluded {n_drop} bead(s) with Feret/Equiv > "
          f"{PSD_MAX_FERET_RATIO} (likely merged clusters).")
diams = diams_full[psd_keep]
vols  = vols_full[psd_keep]
...
# -- Step 13: Export bead labels as TIFF for SPAM DDIC ------------------------
# SPAM's spam-ddic CLI needs a 3D labelled image in a readable TIFF.
import tifffile
os.makedirs(SPAM_DATA_DIR, exist_ok=True)
print(f"\nStep 13: Exporting bead labels for SPAM -> {SPAM_LABELS_PATH}")
if os.path.exists(SPAM_LABELS_PATH):
    try:
        os.remove(SPAM_LABELS_PATH)
    except PermissionError:
        raise PermissionError(
            f"Cannot overwrite {SPAM_LABELS_PATH}; close any program "
            f"holding the file."
        )
tifffile.imwrite(SPAM_LABELS_PATH, bead_labels.astype(np.uint16),
                 photometric='minisblack')
print(f"  Saved: {SPAM_LABELS_PATH}  ({bead_labels.shape}, uint16, n_beads={n_beads})")
