"""Grain tracking for S2 2->3 with a seed that carries the lateral shift and
the radial flow -- kept for the record of how the large-strain sand pairs
were found NOT to be trackable.

Runs under WSL in ~/spam-venv from the DIC session's scratchpad, which holds
_sand_grain_track.py and _sand_track_affine.py:
    /mnt/c/Users/cak7496/AppData/Local/Temp/claude/D--wsl/
        a62987fb-a2df-41c2-a6a3-fd119ddec5c2/scratchpad

WHAT IT SHOWED.  Cross-correlating the support-end grain masks of scan 2 and
scan 3 of S2 gives a lateral shift of (-70, +90) voxels between the two
reconstructions, consistent at three heights; S1 and S2 1->2 show none.
Seeding with that shift and the outline's radial spread gives 6188
reciprocal matches whose head grains move outward by 1.0-1.5 mm, as the
outline does -- but a sweep of the lateral seed offset on S3 returns
5200-6300 reciprocal matches at EVERY offset tried, with the same residual
and a layer-relative motion that never exceeds 116 um (the noise floor is
75 um), while S2 1->2 gives 23855 at the true offset against 9000-10000
elsewhere.  A match count that does not depend on the seed is coincidence,
so the tracked-grain fields for S2 2->3 and S3 1->2 were withdrawn from the
paper and their deformation is read from the outline and the void field.
