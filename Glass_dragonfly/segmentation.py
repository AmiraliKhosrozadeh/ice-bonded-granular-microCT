...
Thresholds from histogram analysis (manual fallbacks):
  Air  (all):   gray < 5301
  Ice:          5301 <= gray < 17579
  Glass beads:  17579 <= gray < 50000
  Aluminum:     gray >= 50000
"""
...
FIRST_SLICE = 98     # 0098.tif
LAST_SLICE  = 961    # 0961.tif  (864 total slices)

# Crop parameters from Dragonfly report1.txt
X_MIN, X_MAX = 565, 1008   # 444 voxels
Y_MIN, Y_MAX = 117, 548    # 432 voxels
...
# Cylinder parameters from Dragonfly (world coordinates in um)
# The cylinder is TILTED -- center X varies with Z.
# Cap1 (top):    (19659.08, 8209.18,   139.36) um
# Cap2 (bottom): (18630.93, 8209.18, 21330.49) um
# Center:        (19145.01, 8209.18, 10734.93) um
# Radius: 5176.45 um,  Height: 21216.06 um
CYL_CAP1_UM = (19501.05, 8206.22,   233.13)   # (X, Y, Z) um
CYL_CAP2_UM = (18737.14, 8178.46, 21259.05)   # (X, Y, Z) um
CYL_RADIUS_UM = 5256.61                    # um

# CT channel origin in um (from Dragonfly: getOrigin() in meters * 1e6)
...
OUT_DIR = r'E:\RPTU-images\CT_images\Glass\Glass_dragonfly\segmentation'
os.makedirs(OUT_DIR, exist_ok=True)

# Dragonfly CT channel search string (for Step 7 ROI creation)
CT_CHANNEL_NAME = 'Glass-1001800'
...
import sys as _sys
_sys.path.insert(0, r'E:\RPTU-images\CT_images\Glass\Glass_dragonfly')
